...
@testitem "Isovec Unitary Fidelity" begin
    ...
    U_H1 = exp(im * Hermitian(randn(2, 2)))
    U_H2 = exp(im * Hermitian(randn(2, 2)))
...
@testitem "Isovec Unitary Fidelity Subspace" begin
    ...
    for ii in test_subspaces
        U_H1 = kron(exp(im * Hermitian(randn(2, 2))), exp(im * Hermitian(randn(2, 2))))
        U_H1_sub = U_H1[ii, ii]
        U_H2 = kron(exp(im * Hermitian(randn(2, 2))), exp(im * Hermitian(randn(2, 2))))
        U_H2_sub = U_H2[ii, ii]
